...
def startprint():
    ...
    print(f'''{Fore.RESET}
                      ...
                      ░█─░█ ░█─░█ ▄█▄ ─░█── ░█▄▄█ ──░█── ░█──▀█
                       ...
                       {Fore.CYAN}Made by 𝖭eаr#0001
                       {Fore.CYAN}Kaitlyn v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Exeter.user.id}   
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send(f"[ERROR]: Missing arguments: {error}", delete_after=1)
    elif isinstance(error, numpy.AxisError):
        await ctx.send('Invalid Image', delete_after=1)
    elif isinstance(error, discord.errors.Forbidden):
        await ctx.send(f"[ERROR]: 404 Forbidden Access: {error}", delete_after=1)
    elif "Cannot send an empty message" in error_str:
        await ctx.send('[ERROR]: Message contents cannot be null', delete_after=1)
    else:
        await ctx.send(f'[ERROR]: {error_str}', delete_after=1)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="𝙆𝘼𝙄𝙏𝙇𝙔𝙉 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
